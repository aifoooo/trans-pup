import type React from 'react';

interface TranslationStatusCardProps {
  type: 'error' | 'success';
  title: string;
  message: string;
}

export const TranslationStatusCard: React.FC<TranslationStatusCardProps> = ({ type, title, message }) => {
  const textColor = type === 'error' ? 'text-red-500' : '';

  return (
    <div className={`flex flex-col text-sm`}>
      <div className="flex flex-row items-center px-4 pb-3 pt-4 font-medium">
        <img
          src="data:image/png;base64,"
          alt="腾讯云"
          className="mr-3 h-5 w-5 rounded-full border border-gray-300 p-0.5"
        />
        <span>{title}</span>
      </div>
      <div className="px-5 pb-5">
        <span className={textColor}>{message}</span>
      </div>
    </div>
  );
};
